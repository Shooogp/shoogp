/* ═══════════════════════════════════════════════════════════════════
   ...
   **سلامةُ البيانات**: فهرسُ الإجابةِ في المدى · لا خيارَ مكرَّراً · عددُ
   ...
   ═══ `dup-right` — ليست مخالفةَ ذوقٍ بل عيبٌ يَرُدُّ إجابةً صحيحة ═══
   في `renderMatching` و`renderLetterPicture` يحملُ عنصرُ العمودِ المختارِ
   ثانياً مفتاحَ **زوجِه** لا نصَّه. فبطاقتانِ متطابقتانِ نصّاً في ذلك العمودِ
   تبدوانِ للتلميذِ سواءً، والوصلُ إلى «الخطأِ» منهما يُرَدُّ وهو صحيحٌ نصّاً.
   ولذلك درجتُها ⛔ لا ⚠️.
   ═══════════════════════════════════════════════════════════════════ */
      ...
      qs.forEach((q, i) => {
        ...
        switch (q.type) {
          ...
          case 'matching': case 'memory': {
            ...
            if (dup(Bb).length) E('dup-right',
              `بطاقتانِ متطابقتانِ في العمودِ الأيمن: ${[...new Set(dup(Bb))].join('، ')} — التوصيلُ الصحيحُ إلى إحداهما يُرَدُّ خطأً`);
          ...
          case 'letter-picture': {
            ...
            if (dup(P.map(x=>x.letter)).length) E('dup-right',
              `بطاقتانِ متطابقتانِ في عمودِ الحروف: ${[...new Set(dup(P.map(x=>x.letter)))].join('، ')} — التوصيلُ الصحيحُ إلى إحداهما يُرَدُّ خطأً`);
            break;
          }
          case 'fill-blank': {
