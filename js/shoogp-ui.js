...
var _SELF_SRC=(document.currentScript && document.currentScript.src) || '';
var IMG_BASE=(function(){
  var b=_SELF_SRC.replace(/[?#].*$/,'').replace(/\/js\/[^\/]*$/,'/images/ui/');
  return /\/images\/ui\/$/.test(b) ? b : 'images/ui/';   /* احتياطٌ آمن */
})();
/* ═══ ختمُ نسخةِ صورِ الإطارات — تُحمَلُ من وسمِ السكربتِ نفسِه ═══
   صورُ الأطرِ تُطلَبُ من هنا لا من HTML، فلم يكن يمسُّها ختمُ `?v=` الذي يكتبُه
   النشرُ في وسومِ HTML (‏.github/workflows/pages.yml) — والمتصفّحُ يخزّنُ الصورةَ
   طويلاً، فيرى المعلّمُ **الإطارَ القديمَ بعدَ تحديثِ ملفِّه ونشرِه بنجاح**
   (وقعَ فعلاً مع `frame-arabic-tall.png` — ٢٠٢٦-٠٨-٠٦).
   الحلُّ بلا مصدرٍ ثانٍ للحقيقة: نلتقطُ `?v=` من **رابطِ هذا السكربتِ نفسِه**
   (‏`js/shoogp-ui.js?v=…` وقد ختمَه النشرُ ببصمةِ النشرة) ونُلحقُه بكلِّ صورة.
   فتتغيّرُ روابطُ الصورِ مع كلِّ نشرةٍ تلقائياً، ولا يُرقَّى شيءٌ يدوياً.
   وإن غابَ الوسمُ (فتحٌ محليٌّ بلا ختم) رجعَ الرابطُ نظيفاً كما كان. */
var IMG_VER=(function(){
  var m=/[?&]v=([^&#]+)/.exec(_SELF_SRC);
  return m ? '?v='+m[1] : '';
})();
function imgURL(name){ return IMG_BASE + name + IMG_VER; }
...
var FRAME_FAMILIES={
  ...
  /* عالمي الرقمي — عائلةُ إطارٍ بهويّةِ «قمرٍ صناعيٍّ ودارات» (قرار المالك ٢٠٢٦-٠٩-١٣):
     ...
     أُعيدَ توليدُ الفريماتِ الأربعةِ ٢٠٢٦-٠٩-١٣ بمواصفاتِ رقم ٢ (§ورقة مواصفات
     فريمات الأسئلة في CLAUDE.md): الحافّةُ اليسرى خاليةٌ تماماً، وشارةٌ نجميّةٌ
     مركزيّةٌ أسفلَ الإطار. l وs وُلِّدا بجيميناي، وm وtall اشتُقّا محلياً من l
     بتوسيعٍ بالخِياطةِ المحسوبةِ (seam carving) يحمي القمرَ الصناعيَّ والشارةَ
     من التشوّه. نِسَبُ النوافذِ المقيسةُ بعدَ الضبط: s ‏2.68 · m ‏1.4046 · l ‏0.99 ·
     tall ‏0.6056 — مطابقةٌ لسلّمِ العائلاتِ القائمةِ (انحرافٌ ≤٠٫٠٢ عن كلِّ هدف).
     ⚠️ **حافّةُ bottom صُحِّحت مرّتَين ٢٠٢٦-٠٩-١٣.** الأولى (خطأ): ظُنَّ أنّ
     مطابقةَ هذه القيمِ لِقياسِ `measureFrameGeo` الحيِّ (الذي تعتمدُه .qfill) تكفي —
     لكنّ القياسَ الحيَّ نفسَه كان معطوباً: يمسحُ **عموداً واحداً أوسطَ** فيصطدمُ بجسمِ
     الشارةِ النجميةِ المركزيةِ المعتمِ فيقفُ عندَه قبلَ الحافّةِ الحقيقية، فوقعت
     الطبقتانِ (`.qwin` هنا و`.qfill` الحيّة) معاً في الخطأِ نفسِه واتّفقتا على رقمٍ
     خاطئ — فبقيَت الفجوةُ الخضراءُ ظاهرةً رغم «تطابق الطبقتين». **والتصحيحُ الحقيقيُّ
     في الدالّةِ نفسِها** (`measureFrameGeo` أدناه): صارت تمسحُ أربعةَ أعمدةٍ (٪٢٠،
     ٣٥، ٦٥، ٨٠ من عرضِ الفتحة) تتفادى المركزَ، وتأخذُ أوسعَ امتدادٍ رأتْه — طابقَ
     ذلك تحليلَ المكوّناتِ المتّصلةِ (bbox حقيقيّ) تماماً. والقيمُ أدناه مأخوذةٌ من
     القياسِ **المُصحَّح** فتتطابقُ الطبقتانِ على الرقمِ الصحيحِ لا الخاطئ. */
  it:{ order:['s','m','l','tall'], flexBase:'l', sizes:{
    s:{img:'frame-it-s.png', ar:'1359 / 577', win:{top:'14.73%', left:'8.09%',  right:'9.34%',  bottom:'12.65%'}, hasFill:false},
    m:{img:'frame-it-m.png', ar:'1189 / 938', win:{top:'21%',    left:'11.94%', right:'16.23%', bottom:'13.86%'}, hasFill:false},
    l:{img:'frame-it-l.png', ar:'958 / 982',  win:{top:'21.08%', left:'14.41%', right:'19.73%', bottom:'13.95%'}, hasFill:false},
    tall:{img:'frame-it-tall.png', ar:'956 / 1417', win:{top:'15.74%', left:'14.23%', right:'19.67%', bottom:'10.59%'}, hasFill:false}
  }}
};
...
function measureFrameGeo(name){
  ...
  im.onload=function(){
    try{
      ...
      /* أربعةُ أعمدةٍ موزَّعةٍ على عرضِ الفتحة (٪٢٠،٣٥،٦٥،٨٠) لا عمودٌ أوسطُ واحد:
         شارةٌ مركزيةٌ معتمةٌ (كشارةِ النجمةِ السفلية في بعضِ الأطر) تصطدمُ بالعمودِ
         الأوسطِ فتُوقفُ المسحَ قبلَ الحافّةِ الحقيقية — مقيسٌ ٢٠٢٦-٠٩-١٣ في أطرِ
         عالمي الرقمي (فرقٌ ٣-٥٫٤٪ في bottom وحدَه). الأعمدةُ الأربعةُ تتفادى المركزَ
         فتُعطي `bt`/`bb` الحقيقيَّين، و`min`/`max` بينها يلتقطُ أوسعَ امتدادٍ رآه
         أيُّ عمودٍ منها — صحيحٌ للإطارِ العاديِّ (تقاربُ الأعمدةِ) وللمعطَّلِ مركزياً سواء. */
      var bt=Infinity, bb=-Infinity;
      [0.2,0.35,0.65,0.8].forEach(function(fr){
        var cxx=Math.round(bl+fr*(br-bl)), t=0,b=0,bv=0,cs2=-1,yy;
        for(yy=0;yy<H;yy++){ a=d[(yy*W+cxx)*4+3];
          if(a<128){ if(cs2<0)cs2=yy; } else if(cs2>=0){ if(yy-cs2>bv){bv=yy-cs2;t=cs2;b=yy-1;} cs2=-1; } }
        if(cs2>=0 && H-cs2>bv){bv=H-cs2;t=cs2;b=H-1;}
        if(t<bt) bt=t;
        if(b>bb) bb=b;
      });
...
/* يبني الإطار بعرض Wf ويعيد هل يسع محتواه (بلا فائض). يحفظ وسمَ qf-hasfill حسب الخانة.
   ...
   ═══ يسعُ **رأسياً وأفقياً** — لا رأسياً وحدَه ═══
   كان الفحصُ رأسياً فقط، فيضيّقُ البحثُ الثنائيُّ الإطارَ إلى أضيقِ عرضٍ **يسعُ طولاً**
   غيرَ عابئٍ بمحتوًى لا ينضغطُ عرضاً — فيُقَصُّ صامتاً. وهذا هو بعينِه ما كان يقعُ في
   «التوصيل»: شبكتُه `1fr 1fr` وللمسارِ `1fr` **حدٌّ أدنى تلقائيٌّ هو `min-content`**، فلا
   يتقلّصُ المسارُ بل **تفيضُ الشبكةُ كلُّها**. مقيسٌ في `g4a2-2-6[4]`: المطلوبُ ‏502px
   (‏210.6 + 219.7 مسارَين + 72 فجوةً) والمتاحُ ‏373px ⇒ فيضٌ ‏129px، والمسارانِ خرجا
   **غيرَ متساويَين** — وهي بصمةُ انهيارِ `1fr` إلى `min-content`.

   **والعلاجُ في اختيارِ الإطارِ لا في ضغطِ المحتوى** (قاعدةُ «الفائضُ فراغٌ لا مطّ»): يتّسعُ
   الإطارُ حتى يسعَ المحتوى بعرضِه الطبيعيّ. ولذلك لم تُمَسَّ الشبكةُ بـ`minmax(0,1fr)` —
   ذاك يسمحُ للمسارِ بالتقلّصِ فيفيضُ **العنصرُ داخلَ المسار** فوقَ فجوةِ خطوطِ التوصيل،
   أي نقلُ الفيضِ لا رفعُه.

   **والإشارةُ `.qbody` وحدَه** — رخيصةٌ (قراءةُ خاصّيتَين) تكفي في السبرِ الحارّ الذي
   يُنادى عشراتِ المرّات: `.qbody` هو القاصُّ فعلاً (‏`overflow:clip`)، وقد قِيسَ أنّ
   **الحالاتِ الثمانيَ في البنكِ كلَّها تُبلِّغُ عنه** فلا تفوتُ واحدةٌ. أمّا `hOverflow`
   الكاملُ (§أدناه) فللتقريرِ والتشخيصِ لا للسبر.

   ⚠️ **والسماحُ هو `overflow-clip-margin` نفسُه لا رقماً مختاراً:** `.qbody` يقصُّ عندَ
   صندوقِه **زائداً هامشَ قصِّه** (20px — وُضِعَ ليخرجَ توهّجُ الأزرارِ كاملاً §الاحتواء)،
   فما دونَ الهامشِ **مرسومٌ كاملاً لا يُقَصُّ منه شيء**. ولو شُدِّدَ الفحصُ إلى بكسلٍ واحدٍ
   لَرفضَ إطاراتٍ سليمةً بفيضٍ لا يُرى: قِيسَ أنّ `g2m-11-2[6]` (شريطٌ متدرّجٌ يفيضُ **4px**
   بلصيقةِ تدريجٍ طرفية) **سقطَ إلى الحاويةِ المرنة** — وهي «ملاذٌ أخيرٌ ونادر»، فبدَّلنا
   قصّاً لا يُرى بخسارةٍ تُرى. فالسماحُ يُقرأُ من الحسابِ الحيِّ لا يُكتَبُ رقماً. */
function clipSlackX(el){
...
function hOverflow(w){
  ...
  for(var i=0;i<all.length;i++){
    ...
    /* بترٌ داخليّ: العنصرُ نفسُه يقصُّ ومحتواه أوسعُ من صندوقِه **زائداً هامشَ قصِّه** —
       والهامشُ يُطرَحُ هنا كما يُطرَحُ في `cut` أعلاه، وإلا تناقضَ المقياسانِ في الملفِّ
       الواحد: يُحتسَبُ بتراً ما لا يُقَصُّ (‏`.qbody` بهامشِ 20px). */
    if(ecs.overflowX==='hidden' || ecs.overflowX==='clip'){
      var s=el.scrollWidth-el.clientWidth-
            (ecs.overflowX==='clip'?(parseFloat(ecs.overflowClipMargin)||0):0);
