...
(function(){
  ...
  const RocketJourney = {
    ...
    _measure: function(){
      ...
      // نصفُ ارتفاعِ النقطةِ يُقرأ حيّاً (offsetHeight بالوحداتِ التصميميّة) لا ثابتاً 8:
      // حجمُ النقطةِ يتبعُ معامِلَ --rj-k في CSS، فأيُّ ثابتٍ هنا يُزيحُها عن المنحنى.
      this.dots.forEach((d,i)=>{ const p=this._pointAt((i+0.5)/N);
        d.style.left  =p.x+'px';
        d.style.bottom=(p.up - (d.offsetHeight||16)/2)+'px'; });
