...
function openBook(key){
  ...
  idx.units.forEach((u,ui)=>{
    /* قفلُ الشراء: مشتقٌّ من رقمِ الوحدةِ وعتبةٍ مجانيةٍ تختلفُ بالمادّة — لا حقلَ
       `locked` في البيانات. القاعدةُ كاملةً في `js/unlock.js` (`freeUnits`)؛ وعددُ
       الوحداتِ يُمرَّرُ لأنّ «ديني حياتي» تقفلُ **الأخيرةَ** وحدَها. */
    const payLocked = !!(window.ShoogpLock && ShoogpLock.isUnitLocked(key, ui, idx.units.length));
  ...
  /* ═══════ إطارُ الوحداتِ المقفلة — قفلٌ واحدٌ لا شارةَ في كلِّ وحدة ═══════
     ...
     الوحداتُ المجانيةُ في الرأسِ (عددُها بالمادّة — `freeUnits` في `js/unlock.js`)
     وما بعدَها مقفلٌ كلُّه. ولذلك سقطتْ حالةُ «وحدةٌ مقفلةٌ تسبقُ مفتوحةً» ومعها
     شارةُ القفلِ المفردة.
     وكتابٌ مفتوحٌ كاملاً (كأحب لغتي ج١) ⇒ `framed` فارغةٌ ⇒ لا إطارَ ولا قفلَ إطلاقاً؛
     وكتابٌ مقفولٌ كاملاً (كأحب لغتي ج٢) ⇒ الإطارُ يضمُّ وحداتِه كلَّها. */
  let lastOpen=-1;
...
const TRACE_LETTERS={
  ...
  /* الحروفُ الكبيرةُ (قرارُ المالك ٢٠٢٦-٠٩-١٨ — درسُ المراجعةِ في كلِّ وحدة): قمّةُ الكبيرِ y=15 */
  S:['M 66 24 C 60 12 36 12 36 28 C 36 44 66 44 66 60 C 66 78 38 78 32 66'],
  A:['M 50 15 L 30 75','M 50 15 L 70 75','M 38 52 L 62 52'],
  T:['M 30 15 L 70 15','M 50 15 L 50 75'],
  P:['M 36 15 L 36 75','M 36 15 C 70 15 70 47 36 47']
};
...
function renderMoney(q, body, fb){
  ...
  /* الفئاتُ العُمانيةُ الحقيقيةُ وحدَها (تصحيحُ المالك ٢٠٢٦-٠٩-٠١): معدنيّاً ٥ و١٠
     و٢٥ و٥٠ بيسة، وورقيّاً ١٠٠ بيسة ونصفُ ريالٍ (٥٠٠) وريالٌ (١٠٠٠). **ولا وجودَ
     لفئةِ ٢٠٠ بيسة** فحُذِفَت من الرفوف؛ ومن احتاجَها كوَّنَها بنقرِ ١٠٠ مرّتَين —
     فقطعةُ الرفِّ تُنقَرُ مراراً وكلُّ نقرةٍ تُضيفُ قطعةً جديدة. */
  const RACK=(q.rack||[5,10,25,50,100,500,1000]).map(v=>Math.round(numOf(v)));
  function fmt(v){ if(v>=1000){ const r=Math.floor(v/1000), b=v%1000;
      return arNum(r)+' ر.ع'+(b?(' و'+arNum(b)+' بيسة'):''); } return arNum(v)+' بيسة'; }
  /* صورُ الفئاتِ الحقيقيةُ (من تصويرِ المعلّمِ — قرارُ المالك ٢٠٢٦-٠٩-٠١): تُعرَضُ
     القطعةُ الفعليةُ بختمِ «نموذج» المخبوزِ في الملفِّ نفسِه (عرفُ النماذجِ التعليميةِ
     للعملات)، وقيمتُها مطبوعةٌ عليها أصلاً فلا نصَّ فوقَها. وصورةُ السلطانِ باقيةٌ
     كما هي **بقرارِ المالكِ الصريحِ** — استثناءٌ من قاعدةِ «بلا ملامحِ وجهٍ» مقصورٌ
     على النقودِ بصيغةِ النموذج. **والفئاتُ السبعُ كلُّها مصوَّرةٌ الآنَ** فلا يُستعمَلُ
     الرسمُ العامُّ أدناه إلا لو أُضيفَت فئةٌ بلا صورة. */
  const MN_PHOTOS={ 5:{img:'عملة-٥-بيسة.png',coin:1}, 10:{img:'عملة-١٠-بيسة.png',coin:1},
                    25:{img:'عملة-٢٥-بيسة.png',coin:1}, 50:{img:'عملة-٥٠-بيسة.png',coin:1},
                    100:{img:'ورقة-١٠٠-بيسة.png'}, 500:{img:'ورقة-نصف-ريال.png'}, 1000:{img:'ورقة-ريال-واحد.png'} };
  function pieceSvg(v){
    const ph=MN_PHOTOS[v];
    if(ph) return ph.coin
      ? '<svg class="mn-svg mn-coin" viewBox="0 0 80 80"><image href="images/'+ph.img+'" x="2" y="2" width="76" height="76" preserveAspectRatio="xMidYMid meet"></image></svg>'
      : '<svg class="mn-svg mn-note" viewBox="0 0 132 76"><image href="images/'+ph.img+'" x="0" y="4" width="132" height="68" preserveAspectRatio="xMidYMid meet"></image></svg>';
    // ≤١٠٠ بيسة تُرسم عملةً معدنيّةً، وما فوقها ورقةً نقديّةً — تمييزٌ شكليٌّ لا لونيٌّ فحسب.
    // وجها القطعتين المرسومان (عناصر عمانية): عملةٌ بحلقةٍ كهرمانيةٍ ووسطٍ فارغٍ وورقةٌ
    // بإطارٍ أخضرَ — القيمةُ نصٌّ فوقَهما. والإجابةُ نقرُ أزرارٍ لا مسارٌ في الرسمِ،
    // فالصورةُ مباحةٌ (§حدُّ المنعِ من الاستبدال)، وتغذيةُ «صحيح» على إطارِ الزرّ.
...
function renderChartRead(q, body, fb){
  ...
  /* أرقامُ التدريجِ في مُنتصفِ عمودٍ خاصٍّ بها يمينَ المحورِ (AX+58، مرساةُ منتصف):
     ...
     ⚠️ والإزاحةُ 58 لا 45 (بلاغٌ ثانٍ من المالك): الفجوةُ تُقاسُ **ببكسلِ الجهازِ بعدَ
     التصغيرِ لا بوحدةِ viewBox** — على منفذِ هاتفٍ (390px) يُصيَّرُ الرسمُ بمقياس ~0.24
     فكانت فجوةُ 25 وحدةً ≈ ٦ بكسلاتٍ فعليةً تُقرأُ التصاقاً. بعدَ التوسعةِ حافّةُ رقمِ
     الخانتَينِ اليسرى عندَ ~863 والمحورُ ينتهي عندَ 822.5 — فجوةُ ٤٠ وحدةً ≈ ١٠ بكسلاتٍ
     على أصغرِ منفذٍ، ورقمُ الثلاثِ خاناتٍ (١٠٠) يبلغُ ~897 فيبقى داخلَ اللوحةِ (900). */
  let grid='';
  for(let v=0; v<=max; v+=step){ const y=py(v);
    grid+='<line class="ch-grid" x1="'+X0+'" y1="'+y.toFixed(1)+'" x2="'+AX+'" y2="'+y.toFixed(1)+'"></line>'+
      '<text class="ch-num" x="'+(AX+58)+'" y="'+(y+10).toFixed(1)+'">'+arNum(v)+'</text>';
  }
