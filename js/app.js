...
/* ===== طبقة النجوم البيضاء داخل بطاقة السؤال (نفس الثيم الفضائي لخلفية الكتاب) =====
   تُملأ نقاطاً بيضاء صغيرة متلألئة، فوق خلفية بلون بطاقة الكتاب الحالي. */
function buildQStars(host, n){
  if(!host) return; n=n||34; let h='';
  for(let i=0;i<n;i++){
    const sz=(Math.random()*2.3+1).toFixed(1);
    h+='<i style="width:'+sz+'px;height:'+sz+'px;left:'+(Math.random()*100).toFixed(1)+'%;top:'+
       (Math.random()*100).toFixed(1)+'%;animation-delay:'+(Math.random()*3).toFixed(1)+'s"></i>';
  }
  host.innerHTML=h;
}
...
function renderQuestions(ls){
  ...
  qs.forEach((q,i)=>{
    ...
    card.className='card-box qcard'+(currentBookColor?' '+currentBookColor:'');
    card.innerHTML=`<div class="qstars"></div>`+
      `<div class="qhead"><span class="qnum">سؤال ${arNum(i+1)}</span><span class="qtype">${Q_LABEL[q.type]||''}</span></div>`+
    ...
    buildQStars(card.querySelector('.qstars'));
