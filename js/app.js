...
function defaultQuestions(){
  return [{ type:'drag-drop', objective:'', prompt:'اسحب اسم كل جزء إلى مكانه على النبتة',
    image:'images/plant.jpg',
    targets:[
      {answer:'الزهرة', box:{x:82,y:12}, dot:{x:62,y:12}},
      {answer:'الورقة', box:{x:16,y:38}, dot:{x:30,y:38}},
      {answer:'الساق',  box:{x:84,y:55}, dot:{x:52,y:55}},
      {answer:'الجذر',  box:{x:18,y:86}, dot:{x:52,y:86}}
    ] }];
}
...
/* ===== زينة بطاقة السؤال بأسلوب الألوان المائية (عناصر عُمانية/تعليمية) =====
   إطار أزرق مائي متموّج يُرسم بالكود، خلفية كريمية ولمسات خضراء، وعناصر واقعية
   (سنبوق، قلعة، علَم عُمان، مجهر، بوصلة، مجسّم أرضي، كيمياء) كصور صغيرة على الحواف. */
const OMANI_ITEMS=[
  {img:'omani-dhow', w:74, css:'top:12px;left:24%'},
  {img:'omani-fort', w:82, css:'top:8px;left:42%'},
  {img:'oman-flag',  w:64, css:'top:14px;left:57%', sm:true},
  {img:'microscope', w:44, css:'top:8px;right:14px'},
  {img:'compass',    w:56, css:'top:27%;left:10px'},
  {img:'globe',      w:58, css:'top:50%;right:12px'},
  {img:'chemistry',  w:92, css:'bottom:10px;left:16px'}
];
const QDECO='<div class="qdeco">'+OMANI_ITEMS.map(function(it){
  return '<img class="omani-el'+(it.sm?' hide-sm':'')+'" src="images/'+it.img+'.png" alt="" style="width:'+it.w+'px;'+it.css+'">';
}).join('')+'</div>';

/* الإطار المتموّج: مسار مستطيل بحوافّ مموّجة وزوايا مدوّرة، يُبنى حسب أبعاد البطاقة */
function wavyRectPath(w,h,inset,amp){
  const r=22, step=4, wl=26, P=[];
  const x0=inset,y0=inset,x1=w-inset,y1=h-inset; let d=0;
  const push=(x,y)=>P.push(x.toFixed(1)+' '+y.toFixed(1));
  const wv=()=>amp*Math.sin(d/wl*6.28318);
  for(let x=x0+r;x<=x1-r;x+=step){push(x,y0+wv());d+=step;}
  for(let a=-90;a<=0;a+=15){const t=a*Math.PI/180;push((x1-r)+r*Math.cos(t),(y0+r)+r*Math.sin(t));}
  for(let y=y0+r;y<=y1-r;y+=step){push(x1-wv(),y);d+=step;}
  for(let a=0;a<=90;a+=15){const t=a*Math.PI/180;push((x1-r)+r*Math.cos(t),(y1-r)+r*Math.sin(t));}
  for(let x=x1-r;x>=x0+r;x-=step){push(x,y1-wv());d+=step;}
  for(let a=90;a<=180;a+=15){const t=a*Math.PI/180;push((x0+r)+r*Math.cos(t),(y1-r)+r*Math.sin(t));}
  for(let y=y1-r;y>=y0+r;y-=step){push(x0+wv(),y);d+=step;}
  for(let a=180;a<=270;a+=15){const t=a*Math.PI/180;push((x0+r)+r*Math.cos(t),(y0+r)+r*Math.sin(t));}
  return 'M'+P.join(' L')+' Z';
}
function buildWavyFrame(card){
  const NS='http://www.w3.org/2000/svg';
  const svg=document.createElementNS(NS,'svg'); svg.setAttribute('class','qframe');
  card.insertBefore(svg, card.firstChild);
  function draw(){
    const w=card.clientWidth, h=card.clientHeight; if(!w||!h) return;
    svg.setAttribute('viewBox','0 0 '+w+' '+h); svg.setAttribute('width',w); svg.setAttribute('height',h);
    const p=wavyRectPath(w,h,11,5);
    svg.innerHTML=
      '<path d="'+p+'" fill="none" stroke="#9dc0e2" stroke-width="11" stroke-linejoin="round" stroke-linecap="round" opacity="0.5"/>'+
      '<path d="'+p+'" fill="none" stroke="#6f9fce" stroke-width="3.4" stroke-linejoin="round" stroke-linecap="round"/>';
  }
...
function renderQuestions(ls){
  ...
  const qs=(window.QUESTIONS && QUESTIONS[ls.file]) || defaultQuestions();
